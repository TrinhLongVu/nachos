...
int Connect()
{
    ...
    remote.sin_family = AF_INET;
    remote.sin_port = htons(ServerPort);
    iRetval = connect(fd,(struct sockaddr *)&remote,sizeof(struct sockaddr_in));
	cout<< iRetval;
    return iRetval;
}
...
void write()
{
	...
	if (fileid[openfileId] != 0)
	{
		int result = fileid[openfileId]->Write(buffer, charcount);
