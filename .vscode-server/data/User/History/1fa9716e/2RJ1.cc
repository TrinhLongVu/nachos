...
void SysPrintString()
{
	...
	bzero(content, MAX_CONTENT);
	readFromMem(content, MAX_CONTENT, addr);
	cout <<content;
...
int SocketTCP()
{
	...
	list[posfreeSocket].socket = fd;

	kernel->machine->WriteRegister(2, posfreeSocket);
	posfreeSocket++;
...
void Open()
{
	...
	list[posfreeFileId].Fileid = kernel->fileSystem->Open(fileName);
	list[posfreeFileId].type = type;

	if (list[posfreeFileId].Fileid && posfreeFileId < 20)
	{
...
void write()
{
	...
	if (list[openfileId].Fileid != 0 && list[openfileId].type == READ_WRITE)
	{
		int result = list[openfileId].Fileid->Write(buffer, charcount);
