...
int Connect()
{
	...
	// // set socketid to connected socket descriptor
	// *socketid = sockfd;
...
void Receive()
{
	...
	shortRetval = recv(socketid, contentSend, sizeContent, 0);
	printf("Response %s\n", contentSend);
	kernel->machine->WriteRegister(2, shortRetval);
}

int i = 0;
OpenFile **fileTable = new OpenFile[20]; 
...
void Open()
{
	...
	OpenFile* id = kernel->fileSystem->Open(fileName);
