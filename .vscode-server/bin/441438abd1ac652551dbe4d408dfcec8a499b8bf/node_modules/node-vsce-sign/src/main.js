...
// Must stay in sync with Microsoft.VisualStudio.Extensions.Signing.ReturnCode (src\Core\ReturnCode.cs).
const ReturnCode = {};

// This comment and its corresponding end tag are for a test.
/* MEMBERS START */
 ...
 */
async function verify(vsixFilePath, signatureArchiveFilePath) {
    const args = ['verify', '--package', vsixFilePath, '--signaturearchive', signatureArchiveFilePath];
    let errorCode;
    let didExecute = false;

    try {
        await execFile(vsceSignFilePath, args);
    } catch (e) {
    ...
    throw new ExtensionSignatureVerificationError(errorCode, didExecute);
}
